import { Q, SCALE, q, clampQ, qMul, mulDiv, cbrtQ, sqrtQ } from "./units";
...
export function derivePeakForceEff_N(a: IndividualAttributes): number {
  ...
  return mulDiv(F0, combined as number, SCALE.Q);
}
...
export function deriveMaxAcceleration_mps2(a: IndividualAttributes, tractionCoeff: Q): number {
  ...
  const tractionLimit_N = mulDiv(normalForce_N_scaled, tractionCoeff as number, SCALE.Q);
...
export function deriveMaxSprintSpeed_mps(a: IndividualAttributes): number {
  ...
  return mulDiv(mult as number, SCALE.mps, SCALE.Q);
}
...
export function deriveJumpHeight_m(a: IndividualAttributes, reserveSpend_J: number): number {
  ...
  const controlFactor = q(0.7) + qMul(q(0.3), a.control.controlQuality);
  const Eeff = mulDiv(mulDiv(Euse, a.performance.conversionEfficiency as number, SCALE.Q), controlFactor as number, SCALE.Q);
...
export function deriveMovementCaps(
  ...
  return {
    maxSprintSpeed_mps: mulDiv(baseV, speedMul as number, SCALE.Q),
    maxAcceleration_mps2: mulDiv(baseA, accelMul as number, SCALE.Q),
    jumpHeight_m: mulDiv(baseH, jumpMul as number, SCALE.Q),
  };
}
...
export function stepEnergyAndFatigue(
  ...
  const P = mulDiv(demandedPower_W, demandMul as number, SCALE.Q);
  const dt = dt_s;
