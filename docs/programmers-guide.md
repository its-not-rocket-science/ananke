...
> **Full project documentation:** [github.com/its-not-rocket-science/ananke](https://github.com/its-not-rocket-science/ananke)
...
```typescript
import type { BodyRegion } from "@its-not-rocket-science/ananke";
...
```typescript
import {
  CollectingTrace, stepWorld, renderChronicle,
} from "@its-not-rocket-science/ananke";
...
Advance a polity through 90 days with plague, war, and tech diffusion:

```typescript
import {
  createPolityRegistry, stepPolityDay, applyEmotionalContagion,
  stepTechDiffusion, createEmotionalWave, FEAR_WAVE,
} from "@its-not-rocket-science/ananke";
...
for (const p of registry.polities) {
  console.log(`${p.name}: pop=${p.population} era=${p.techEra} morale=${p.moraleQ / SCALE.Q}`);
}
```
...
`AMATEUR_BOXER`, `LARGE_PACIFIC_OCTOPUS`, and all species from `src/species.ts`
(humans, elves, dwarves, orcs, dragons, Vulcans, Klingons, …).
...
// Or: treat a wounded ally
commands.set(mediclId, {
  kind:     "treat",
  targetId: woundedId,
...
identical `WorldState` at every tick, regardless of platform, JS engine, or execution
time.
...
// Deterministic random number inside your AI/event code:
...
const json = serializeReplay(recorder.replay);  // stable JSON
...
Extract per-segment pose data for driving a humanoid rig:
...
See [`docs/bridge-contract.md`](bridge-contract.md) for the full double-buffer protocol
and `AnimationHints` field-by-field contract.
...
constructor and `SCALE` constants make the intent clear.
...
Full benchmark methodology: [`docs/performance.md`](performance.md)
...
- **Isolated sub-system validation:** `npm run run:validation` — compares physical
  constants against sport-science and biomechanics datasets
- **Emergent validation:** `npm run run:emergent-validation` — four historical combat
  scenarios (du Picq, Keegan, Lanchester, Raudzens) across 100 seeds each
- **Pinned baseline:** [`docs/emergent-validation-report.md`](emergent-validation-report.md)
  — committed result summaries that CI guards against regression
...
|---|---|
| [`docs/host-contract.md`](host-contract.md) | Stable integration surface — everything needed to embed Ananke without reading `src/` |
| [`docs/integration-primer.md`](integration-primer.md) | Data-flow diagrams, type glossary, gotchas |
| [`docs/bridge-contract.md`](bridge-contract.md) | 3D renderer bridge protocol (AnimationHints, GrapplePoseConstraint) |
| [`STABLE_API.md`](../STABLE_API.md) | Full tier table for every export |
| [`docs/versioning.md`](versioning.md) | Semver policy, breaking-change tiers, upgrade cadence |
| [`docs/performance.md`](performance.md) | Benchmark results, operational guide, entity caps |
| [`docs/emergent-validation-report.md`](emergent-validation-report.md) | Historical scenario validation report |
| [GitHub README](https://github.com/its-not-rocket-science/ananke#readme) | Full project overview, implementation status, entity model reference |
