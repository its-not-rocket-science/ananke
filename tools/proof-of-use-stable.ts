...
import { serializeBridgeFrame } from "../src/host-loop.js"; // Tier 2 (optional)
import type { Command, CommandMap, Entity, KernelContext, WorldState } from "../src/index.js";
...
function inferEndReason(world: WorldState): string {
...
function nearestEnemy(world: WorldState, actor: Entity): Entity | undefined {
  let nearest: Entity | undefined;
  let bestDistSq = Number.POSITIVE_INFINITY;

  for (const other of world.entities) {
    if (other.id === actor.id || other.injury.dead || other.teamId === actor.teamId) continue;
    const dx = (other.position_m.x - actor.position_m.x) / SCALE.m;
    const dy = (other.position_m.y - actor.position_m.y) / SCALE.m;
...
function buildHostCommands(world: WorldState): CommandMap {
  ...
  for (const entity of world.entities) {
    ...
    const dx = (target.position_m.x - entity.position_m.x) / SCALE.m;
    const dy = (target.position_m.y - entity.position_m.y) / SCALE.m;
    const norm = Math.hypot(dx, dy);

    const move: Command = {
      kind: "move",
      dir: { x: q(norm > 0 ? dx / norm : 0), y: q(norm > 0 ? dy / norm : 0), z: q(0) },
      intensity: q(1),
      mode: "run",
    };

    const attackNearest: Command = {
      kind: "attackNearest",
      mode: "strike",
      intensity: q(1),
    };

    commands.set(entity.id, [move, attackNearest]);
  }
...
<head>
<meta charset="utf-8" />
<title>Ananke Proof-of-Use Inspector (Stable)</title>
<style>
...
<body>
<h1>Ananke — Proof-of-Use Inspector (Stable Host Path)</h1>
<div class="card">
  <h2>Scenario</h2>
  <pre id="scenario"></pre>
</div>
...
<div class="card">
  ...
  <small>Tier 1 host loop + optional Tier 2 bridge serializer (host-loop).</small>
  <table>
    <thead>
...
async function run(): Promise<void> {
  ...
  const outDir = resolveRepoPath("artifacts", "proof-of-use", "stable");
  ...
  for (let i = 0; i < scenario.maxTicks; i++) {
    const cmds = buildHostCommands(world);
  ...
  const summary: OutcomeSummary = {
    ...
    policy: "host-level nearest-enemy policy (no internal AI modules)",
    tierUsage: {
      tier1Root: ["loadScenario", "stepWorld", "q", "SCALE", "ReplayRecorder", "serializeReplay", "replayTo"],
      tier2Optional: ["serializeBridgeFrame (from host-loop subpath)"],
    },
  };
  ...
  console.log("ANANKE PROOF-OF-USE (STABLE)");
...
run().catch((err) => {
  console.error("proof-of-use stable failed", err);
  process.exitCode = 1;
});
