...
import { loadScenario, stepWorld, q, SCALE } from "../src/index.js";
import { serializeBridgeFrame } from "../src/host-loop.js";
import { ReplayRecorder, serializeReplay, replayTo } from "../src/replay.js";
...
<head>
<meta charset="utf-8" />
<title>Ananke Proof-of-Use Inspector</title>
<style>
...
<body>
<h1>Ananke — Proof-of-Use Inspector</h1>
<div class="card">
  <h2>Scenario</h2>
  <pre id="scenario"></pre>
</div>
...
<div class="card">
  ...
  <small>Bridge frame source: serializeBridgeFrame() stable host path.</small>
  <table>
    <thead>
...
async function run(): Promise<void> {
  ...
  const outDir = resolveRepoPath("artifacts", "proof-of-use");
  ...
  console.log("ANANKE PROOF-OF-USE SLICE");
...
run().catch((err) => {
  console.error("proof-of-use slice failed", err);
  process.exitCode = 1;
});
